...
let modalContents = select('.modal-contents');
let modalTitle = select('.title-modal span');
const url =
  'https://gist.githubusercontent.com/Miserlou/c5cd8364bf9b2420bb29/raw/2bf258763cdddd704f8ffd3ea9a3e81d25e2c6f6/cities.json';
let count = 0;

//function fetch
const fetchResult = (api) => {
...
search.onkeyup = () => {
  ...
  if (search.value !== '' && search.value.length > 1) {
    fetchResult(url)
      .then((myJson) => {
        ...
        if (count == 0) {
          result_containner.innerHTML = `
              ...
              <p>Your search - <b>${search.value}</b> - did not match any City or State.</p>
      ...
      .catch((error) => {
        result_containner.innerHTML = `<div class="no-data"> <p>Oops!, Something went wrong!, <b>${error}</div>`;
      });
...
const modalDiv = (Rank) => {
  fetchResult(url)
    .then((myJson) => {
      for (const key in myJson) {
        ...
        if (result.rank == Rank) {
          ...
          modalTitle.innerHTML = result.city;
          displayModal();
    ...
    .catch((error) => {
      modalContents.innerHTML = `<div class="no-data"> <p>Oops!, Something went wrong!, <b>${error}</div>`;
    });
};
...
const search_data = (value, key) => {
  ...
  key = filterText(key);
  return value.search(key) !== -1 ? true : false;
};
...
select('.modal-close').addEventListener('click', () => {
  displayModal();
});
